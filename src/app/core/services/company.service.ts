...
export class CompanyService {
  ...
  // Always the owner's currently ACTIVE company (see #switch below) —
  ...
  // A second (or third…) company under the same owner login — capped by
  ...
  // Every company this owner runs — the navbar switcher's data source.
  list(): Observable<{ companies: CompanySummary[] }> {
    return this.http.get<{ companies: CompanySummary[] }>(`${API_BASE_URL}/companies`);
  }

  // Moves the owner's session onto another of their OWN companies.
