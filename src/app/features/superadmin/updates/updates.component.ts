...
@Component({
  selector: "app-superadmin-updates",
  standalone: true,
  imports: [
...
export class SuperadminUpdatesComponent implements OnInit {
  ...
  load(): void {
    ...
    this.service.listSuperadmin().subscribe({
      next: (res) => {
  ...
  onFilesSelected(event: Event): void {
    ...
    if (combined.length > MAX_FILES) {
      this.fileError.set(this.translate.instant("superadmin.updates.too_many_files", { max: MAX_FILES }));
      return;
    }
    const invalid = picked.find((f) => !ALLOWED_TYPES.includes(f.type) || f.size > MAX_FILE_SIZE);
    if (invalid) {
      this.fileError.set(this.translate.instant("superadmin.updates.invalid_file", { name: invalid.name }));
      return;
    }
  ...
  submit(): void {
    ...
    this.service.create(this.version().trim(), this.title().trim(), this.description().trim(), this.files()).subscribe({
      next: (res) => {
        ...
        this.toast.success(this.translate.instant("superadmin.updates.published"));
      },
      error: (err) => {
        this.saving.set(false);
