...
describe("SettingsSectionsService", () => {
  ...
  it("a non-admin never sees an adminOnly section", () => {
    build("staff");
    expect(service.sections().every((s) => !s.adminOnly)).toBe(true);
  });

  it("leaves a non-admin with nothing but Appearance — Settings is the admin's alone", () => {
    build("staff");
  ...
  it("isVisible defaults to true for a section with neither adminOnly nor a permission", () => {
    build("staff");
  ...
  it("offers import/export as a settings section, for the admin only", () => {
    build("admin");
