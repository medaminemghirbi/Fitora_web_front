...
import { AdminUpdatesComponent } from "./updates.component";
...
describe("AdminUpdatesComponent", () => {
  ...
  beforeEach(async () => {
    service = jasmine.createSpyObj<AppUpdatesService>("AppUpdatesService", ["listAdmin", "create"]);
    service.listAdmin.and.returnValue(
      of({
        app_updates: [
          {
            id: "u1",
            ...
            created_by: { id: "a1", full_name: "Admin A" },
            media: [],
          },
        ],
      })
    ...
    await TestBed.configureTestingModule({
      imports: [AdminUpdatesComponent, TranslateModule.forRoot()],
  ...
  it("sets the error flag when loading fails", () => {
    service.listAdmin.and.returnValue(throwError(() => new Error("nope")));
    component.load();
  ...
  it("onFilesSelected rejects a disallowed file type", () => {
    ...
    expect(component.files().length).toBe(0);
    expect(component.fileError()).toBe("admin.updates.invalid_file");
  });
  ...
  it("onFilesSelected rejects more than the max file count", () => {
    ...
    expect(component.files().length).toBe(0);
    expect(component.fileError()).toBe("admin.updates.too_many_files");
  });
